#!/usr/bin/env bash
# install-terax-themes.sh — Install 12 custom themes for Terax
...
case "$(uname -s)" in
  Linux)  CONFIG_DIR="${XDG_CONFIG_HOME:-$HOME/.config}/com.terax.app/themes" ;;
  Darwin) CONFIG_DIR="$HOME/Library/Application Support/com.terax.app/themes" ;;
  *MINGW*|*MSYS*|*CYGWIN*)
    CONFIG_DIR="$(cygpath -u "$APPDATA")/com.terax.app/themes" ;;
  *)
    echo "Unsupported platform: $(uname -s)" >&2
    echo "Please manually copy .terax-theme files to the Terax themes directory." >&2
    exit 1
    ;;
esac
...
# ─── Theme definitions ──────────────────────────────────────────────────────
write_theme() {
  local file="$CONFIG_DIR/$1.terax-theme"
  shift
  printf '%s\n' "$*" > "$file"
  echo "  ✓ $1"
}

echo "Installing Terax themes to: $CONFIG_DIR"
echo ""
...
echo ""
echo "────────────────────────────────────────"
echo "  12 themes installed successfully!"
echo "  Restart Terax and find them under"
