...
if command -v gsettings >/dev/null 2>&1; then
  ...
  echo "$STRIPPED" | tr ',' '\n' | sed 's/^ *//;s/ *$//' | while read -r uuid; do
    case "$uuid" in
      "$UUID_X"|"$UUID_XMADRID"|"$UUID_XLAHABANA"|"$UUID_XSEUL"|"$UUID_XMIAMI"|"$UUID_XPARIS"|"$UUID_XTOKIO"|"$UUID_XOSLO"|"$UUID_XHELSINKI"|"$UUID_XBERLIN"|"$UUID_XLONDON"|"$UUID_XPRAGA"|"$UUID_XBOGOTA")
        ;;
      *)
        if [ -z "$FIRST" ]; then
          FIRST="$uuid"
