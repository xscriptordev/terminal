...
if command -v gsettings >/dev/null 2>&1; then
  ...
  echo "$STRIPPED" | tr ',' '\n' | sed 's/^ *//;s/ *$//' | while read -r uuid; do
    case "$uuid" in
      "$UUID_XSCRIPTOR"|"$UUID_XSCRIPTOR_LIGHT"|"$UUID_XRETRO"|"$UUID_XDARKCANDY"|"$UUID_XCANDYPOP"|"$UUID_XSENSE"|"$UUID_XSUMMER"|"$UUID_XNORD"|"$UUID_XNORDINV"|"$UUID_XGREYSCALE"|"$UUID_XGREYSCALEINV"|"$UUID_XDARKCOLORS"|"$UUID_XPERSECUTION")
        ;;
      *)
        if [ -z "$FIRST" ]; then
          FIRST="$uuid"
