...
restore_config_file() {
  ...
  if grep -Eq '^import[[:space:]]*=\s*\[.*(xscriptor-theme\.toml|xscriptor-theme-light\.toml|x-retro\.toml|x-dark-candy\.toml|x-candy-pop\.toml|x-sense\.toml|x-summer-night\.toml|x-nord\.toml|x-nord-inverted\.toml|x-greyscale\.toml|x-greyscale-inverted\.toml|x-dark-colors\.toml|x-persecution\.toml).*\]' "$FILE"; then
